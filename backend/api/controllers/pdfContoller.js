...
const b1FormItems = [
    {
        id: "b1.1",
        description: "Have you omitted any disclosures due to sensitive or classified information? If yes, specify which ones and provide the justification.",
    },
    {
        id: "b1.2",
        description: "Is your sustainability report prepared on a consolidated basis (including subsidiaries) or on an individual basis?",
    },
    {
        id: "b1.3",
        description: "If reporting on a consolidated basis, please list all subsidiaries included in the report along with their registered addresses.",
    },
    {
        id: "b1.4",
    ...
    {
        id: "b1.12",
        description: "Does your company hold any sustainability-related certifications or labels? If yes, please list them along with issuer, date, and rating/score.",
    },
];
const b2FormItems = [
    {
        id: "b2.1",
        description: "What specific practices does your company have in place to reduce water consumption, energy use, or greenhouse gas emissions? Please briefly describe each practice.",
    },
    {
        id: "b2.2",
        description: "Does your company have formal sustainability policies? If yes, please list and briefly describe them.",
    },
    {
        id: "b2.3",
    ...
    {
        id: "b2.4",
        description: "What future initiatives or plans does your company have to enhance sustainability (e.g., planned projects, investments, or strategic changes)?",
    },
    {
        id: "b2.5",
    ...
    {
        id: "b2.6",
        description: "How do you monitor progress towards these targets (describe monitoring methods, tools, or indicators)? Provide quantitative progress against each target (if available).",
    },
];
class PdfController {
}
...
_a = PdfController;
PdfController.generateGeneralInfoReport = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { userId } = req.params;
    const { companyName, reportingYear } = req.query;
    // Fetch B1 and B2 forms for the user
    const [b1Form, b2Form] = await Promise.all([
        GeneralInfo_1.GeneralInfo.findOne({ userId, formType: forms_1.FormType.GENERAL_INFO_B1 }),
        GeneralInfo_1.GeneralInfo.findOne({ userId, formType: forms_1.FormType.GENERAL_INFO_B2 }),
    ]);
    if (!b1Form && !b2Form) {
        throw new errorHandler_1.AppError("No general information forms found for this user", 404);
    }
    // Create PDF document
...
async function generatePdfContent(doc, data) {
    ...
    function addHeader(text, size = 16, color = colors.primary) {
    ...
    function addText(text, size = 11, color = colors.text, indent = 0) {
        const options = {
    ...
    if (Object.keys(b1Data).length > 0) {
        ...
        addText("This section covers the fundamental information about your company and the basis for preparing this sustainability report.", 12, colors.secondary);
        addDivider();
    ...
    if (Object.keys(b2Data).length > 0) {
        ...
        addText("This section focuses on your company's current sustainability practices and future plans for transitioning towards a more sustainable economy.", 12, colors.secondary);
        addDivider();
